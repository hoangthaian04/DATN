...
export const Dashboard: React.FC = () => {
  const { user } = useAuth();
  const shouldShowProfileReminder = user?.profileCompleted === false;
    ...
    <div className="flex-1 p-8 bg-[#F8FAFC] min-h-screen">
      ...
      <div className="max-w-[1440px] mx-auto space-y-6">
        {shouldShowProfileReminder && (
          <div className="flex flex-col gap-4 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-900 shadow-sm sm:flex-row sm:items-center sm:justify-between">
            <div className="flex gap-3">
              <AlertCircle className="mt-0.5 h-5 w-5 shrink-0 text-amber-600" />
              <div>
                <p className="text-sm font-extrabold">Hồ sơ doanh nghiệp của bạn chưa hoàn thiện.</p>
                <p className="mt-1 text-xs font-semibold text-amber-700">
                  Hoàn thiện thông tin công ty để Career Site và các luồng tuyển dụng hiển thị đầy đủ hơn.
                </p>
              </div>
            </div>
            <Link
              to="/onboarding"
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-600 px-4 py-2.5 text-xs font-extrabold text-white shadow-sm transition hover:bg-amber-700"
            >
              Hoàn thiện hồ sơ <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
        )}
