...
export const EditJobModal: React.FC<Props> = ({ isOpen, onClose, jobData, onSave, isPending }) => {
  ...
  const next = () => {
    ...
    if (job.startDate && job.endDate && job.endDate < job.startDate) {
      toast.error('Ngày kết thúc nhận hồ sơ phải lớn hơn hoặc bằng ngày bắt đầu.');
      return;
    }
    setStep(2);
  };
...
const Input = ({ label, value, onChange, required, type = 'text', placeholder, min, className = '' }: { label: string; value: string; onChange: (value: string) => void; required?: boolean; type?: string; placeholder?: string; min?: number | string; className?: string }) => <label className={`block space-y-1.5 ${className}`}><span className="text-[10px] font-bold uppercase tracking-wide text-slate-500">{label} {required && '*'}</span><input required={required} type={type} min={min ?? (type === 'number' ? 0 : undefined)} value={value} onChange={event => onChange(event.target.value)} placeholder={placeholder} className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs font-semibold text-slate-700 outline-none focus:border-primary-500 focus:bg-white" /></label>;
